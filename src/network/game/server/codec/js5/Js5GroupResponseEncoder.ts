...
import type Js5GroupResponse from '#/network/game/server/model/js5/Js5GroupResponse.ts';
import Js5ServerProt from '#/network/game/server/prot/js5/Js5ServerProt.ts';

export default class Js5GroupResponseEncoder extends ServerGameMessageEncoder<Js5GroupResponse> {
    prot = Js5ServerProt.PREFETCH_REQUEST;

    encode(buf: Packet, message: Js5GroupResponse): void {
        if (message.xor != 0) {
