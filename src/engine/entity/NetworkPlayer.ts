...
import type { WealthEventParams } from '#/engine/entity/tracking/WealthEvent.js';
...
export class NetworkPlayer extends Player {
    ...
    static inBuf = Packet.alloc(5000);
    ...
    read(): boolean {
        ...
        if (this.client.opcode === -1) {
            ...
            if (this.client.decryptor) {
                this.client.opcode = (NetworkPlayer.inBuf.g1() - this.client.decryptor.takeNextValue()) & 0xff;
            } else {
    ...
    writeInner(message: ServerGameMessage): void {
        ...
        if (client.encryptor) {
            buf.p1(prot.id + client.encryptor.takeNextValue());
        } else {
            buf.p1(prot.id);
        }
