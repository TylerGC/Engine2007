...
import { PlayerInfoProt } from '#/network/rsbuf/index.ts'
...
import { isClientConnected } from '#/engine/entity/ClientConnection.js';
...
import type { WealthTransactionEvent, WealthEvent } from '#/engine/entity/tracking/WealthEvent.js';
...
class World {
    ...
    async start(skipMaps = false, startCycle = true): Promise<void> {
        ...
        const objIndex = await OpenRs2.RS2_500.loadLocalPackedIndex(19);
        if (objIndex) {
        ...
        const SeqIndex = await OpenRs2.RS2_500.loadLocalPackedIndex(20);
        if (SeqIndex) {
        ...
        if (!Environment.STANDALONE_BUNDLE) {
            printInfo(kleur.green().bold('World ready') + kleur.white().bold(' (use the Java client.. if you want to)'));
        }
        ...
        player: for (const player of this.newPlayers) {
            ...
            if (player.reconnecting) {
                for (const other of this.players) {
                    ...
                    if (isClientConnected(other) && isClientConnected(player)) {
    ...
    static loginBuf = Packet.alloc(5000);
